    ...
    *started* with the backup number incremented. The stdlib handler
    ...
    def doRollover(self):
        ...
        if self.backupCount < 1:
            ...
            self.baseFilename = str(self.directory / f"{self.baseStem}-part{latest_part_num + 1}.log")
...
def init_logging(level: int, location: pathlib.Path) -> None:
    ...
    dpy_logger = logging.getLogger("discord.http")
    dpy_logger.setLevel(logging.WARNING)
    dpy_logger = logging.getLogger("discord.state")
    dpy_logger.setLevel(logging.WARNING)
    warnings_logger = logging.getLogger("py.warnings")
    warnings_logger.setLevel(logging.WARNING)
    core_logger = logging.getLogger("core.bot")
    core_logger.setLevel(logging.INFO)
    dpy_warnings_logger = logging.getLogger("discord.errors")
    dpy_warnings_logger.setLevel(logging.WARNING)
    ...
    for path in location.iterdir():
        if match := re.match(r"latest(?P<part>-part\d+)?\.log", path.name):
            part = match.groupdict(default="")["part"]
            latest_logs.append((path, part))
        if match := re.match(r"previous(?:-part\d+)?.log", path.name):
